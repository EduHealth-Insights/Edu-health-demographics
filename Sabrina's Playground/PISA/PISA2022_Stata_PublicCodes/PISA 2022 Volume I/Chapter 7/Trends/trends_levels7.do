...
use cnt *sch*id *st*id* `group' lp_pv*`domn'_l tp_pv*`domn'_l w_* using "`database'"

repest PISA , estimate(means lp_pv@`domn'_l tp_pv@`domn'_l, pct)  over(`group', test) by(cnt, levels(`countries')) outfile(`outfile') `repestoptions' `coverage'
