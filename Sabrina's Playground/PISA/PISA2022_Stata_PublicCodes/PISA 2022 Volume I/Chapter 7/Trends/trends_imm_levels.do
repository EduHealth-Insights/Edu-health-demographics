...
** Catch arguments
args jobN database domn outfile countries repestoptions coverage stat group
...
use *cnt *sch*id *st*id* `stat' `group' w_* using "`database'"
cap ren  fcnt* cnt*

repest PISA , estimate(means `stat', pct)  over(`group', test) by(cnt, levels(`countries')) outfile(`outfile') `repestoptions' `coverage'
