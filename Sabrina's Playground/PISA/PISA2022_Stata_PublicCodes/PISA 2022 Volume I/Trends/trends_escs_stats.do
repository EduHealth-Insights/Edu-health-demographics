...
** Catch arguments
args jobN database domn outfile countries  repestoptions betalog escs
foreach arg in repestoptions betalog escs {
	if "``arg''" == "NULL" local `arg' ""
	}
	
** Define FilePaths (global)
run "c:\temp\Trends\infile\trends_ENV.do"
cd "${sandbox}"
...
use cnt *sch*id *st*id* `escs'_q pv*`domn' w_* using "`database'"
		repest PISA , estimate(summarize pv@`domn', stats(mean p10 p90 idr)) over(`escs'_q, test) by(cnt, levels(`countries')) outfile(`outfile') `betalog' `repestoptions'
